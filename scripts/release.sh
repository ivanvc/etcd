...
main() {
  ...
  if [ "${DRY_RUN}" == "true" ] || [ "${NO_GH_RELEASE}" == 1 ]; then
    ...
    local release_version_major_minor=${RELEASE_VERSION%.*}
    local release_version_major=${release_version_major_minor%.*}
    local release_version_minor=${release_version_major_minor/*./}

    # Disable sellcheck SC2016, the single quoted syntax for sed is intentional.
    # shellcheck disable=SC2016
    sed 's/${RELEASE_VERSION}/'"${RELEASE_VERSION}"'/' ./scripts/release_notes.tpl.txt |
      sed 's/${RELEASE_VERSION_MAJOR_MINOR}/'"${release_version_major_minor}"'/' |
      sed 's/${RELEASE_VERSION_MAJOR}/'"${release_version_major}"'/' |
      sed 's/${RELEASE_VERSION_MINOR}/'"${release_version_minor}"'/' > "${release_notes_temp_file}"
    ...
    maybe_run gh --repo "${gh_repo}" release upload "${RELEASE_VERSION}" ./release/SHA256SUMS
