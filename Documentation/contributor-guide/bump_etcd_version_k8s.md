...
You can use this [issue](https://github.com/kubernetes/kubernetes/issues/131101) as a reference when updating the etcd version in Kubernetes. However, note that it's not required to create an issue.
...
You can refer to [Kubernetes' guide on Adding or updating a dependency](https://github.com/kubernetes/community/blob/master/contributors/devel/sig-architecture/vendor.md#adding-or-updating-a-dependency).

1. As a reference, you can query which dependencies need to be updated:
...
2. Update the version, replace `<NEW_VERSION>` with the version prefixed with `v` (i.e., `v3.5.22`).

    ```bash
    grep 'go.etcd.io/etcd/.*/v3' go.mod | awk '{print $1}' | xargs -I{} hack/pin-dependency.sh {} <NEW_VERSION>
    ```
...
4. Check if the new dependency requires newer versions of existing dependencies we have pinned. You can check this by:
    ...
    * Checking if any new `replace` directives were added to `go.mod` files of components inside the staging directory. You can inspect this by running:
        ```
        git diff 'staging/**/*/go.mod'
        ```
